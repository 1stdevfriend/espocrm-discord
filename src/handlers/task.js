...
function createTaskEmbed(data, eventType, baseEmbed) {
  let title = `${EMOJIS.task} Task ${capitalize(eventType)}`;
  ...
  if (eventType === 'create') {
    fields = [
      ...
      { name: 'Assigned User', value: data.assignedUserName || 'N/A', inline: true },
      { name: 'Description', value: data.description || 'N/A', inline: false }
    ];
  } else if (eventType === 'update') {
    fields = [
      { name: 'Task ID', value: data.id || 'N/A', inline: true },
      ...Object.entries(data)
        .filter(([key]) => key !== 'id')
